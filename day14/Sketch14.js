...
function setup() {
  ...
  mysymbol = new MySymbol(width / 2, height / 2); //position
...
function draw() {
  background(0,206,209); //this color like sea

  for (const item of textList) {
    item.updateScale(sin(frameCount / 10 + item.x) * 0.1 + 0.1);//here is for the little symbol's size and moving.
    item.draw();
  }
}
