...
function setup() {
  ...
  for (let x = 0; x < pg.width; x += 30) {
    for (let y = 0; y < pg.height; y += 30) { //shape
...
function draw() {
  background(0);
  if (frameCount % 60 == 0) { //I have tried many values and 60 is the smoothest one
    pg.background(0);
  }
  ...
  for (const item of textList) {
    ...
    item.updatePos(
      ...
      item.y + cos(frameCount / 10) * 3 //draw a circle here, any change in value may turn into a line
    );
    item.draw(pg);
  }
