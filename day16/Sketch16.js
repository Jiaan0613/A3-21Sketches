...
function setup() {
  ...
  for (let x = 0; x < pg.width; x += 30) {
    for (let y = 0; y < pg.height; y += 30) { //The smaller the number, the more dense the lines will be
...
function draw() {
  ...
  for (const item of textList) {
    ...
    item.updatePos(item.x + sin(frameCount / 10) * 3, item.y); //the movememt and the length
    item.draw(pg);
  }
